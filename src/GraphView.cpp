...
void GraphView::mouseReleaseEvent ( QMouseEvent * e ){
    QGraphicsItem * item = minItem( e->pos());
    //        foreach( QGraphicsItem * t, itms ){
    //            qDebug()<<"items: "<<(void*)t;
    //        }
...
QGraphicsItem * GraphView::minItem(const QPointF &pos)
{
    qreal dist, minDist = 99999;
    QGraphicsItem * min = 0;
...
//    }
    foreach(QGraphicsItem *item , list ){
        if( item->type() == EdgeType ){
            ...
            dist = edge->Distance( /*edge->*/mapToScene( pos.toPoint() ) );
//            qDebug()<<"Edge "<<edge->edge()->info()->id()<<":  "<<dist;
            if( minDist > dist ){
                min = edge;
                minDist = dist;
//                qDebug()<<"min";
